...
module.exports = {
  ...
  subscribe: async (ctx) => {
    ...
    return await strapi.query("training").update(
      { id },
      {
        pets_completed: [...entity.pets_completed, ctx.state.user.current_pet],
      }
    );
  },
  unsubscribe: async (ctx) => {
    const { id } = ctx.params;
    let entity = await strapi.query("training").findOne({ id });
    let subscribes = entity.pets_completed.filter((p) => {
      return p.id != ctx.state.user.current_pet;
    });
    await strapi.config.functions["mixin"].updatePetTimeline(
      ctx.state.user.current_pet,
      {
    ...
    return await strapi
      .query("training")
      .update({ id }, { pets_completed: subscribes });
  },
  find: async (ctx) => {
    var trainings = await strapi.query("training").find();
    trainings = trainings.filter((train) => {
      return (
        train.pets_completed.filter((u) => u.id === ctx.state.user.current_pet)
          .length === 0
      );
    });
